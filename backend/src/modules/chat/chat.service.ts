...
export class ChatService {
  ...
  async processMessage(userId: string, dto: SendMessageDto): Promise<ChatResponse> {
    ...
    // 性能优化：异步保存用户消息（不阻塞 API 调用）
    console.log("[ChatService] 异步保存用户消息...");
    this.saveUserMessage(userId, dto.message).catch((err) => {
      this.logger.error("保存用户消息失败", err);
    });
    ...
    if (hasApiKey) {
      // 精简高效的系统提示词（保留专业性，去除冗余和 emoji）
      const systemPrompt = `你是DietLens膳食管家，20年临床营养师+米其林主厨背景。

专业回答规则：
1. 营养术语：GI值、抗氧化剂、必需氨基酸、支链氨基酸、Omega-3、BCAA等
2. 语气温暖：像关怀备至的私人健康顾问
3. 食谱格式：
   【菜品】充满食欲的命名
   【热量】xxx kcal | 蛋白质xxg 碳水xxg 脂肪xxg
   【食材】精确到克
   【做法】关键步骤+口感秘籍
   【营养师点评】适合人群与营养原理
4. 仅回答健康饮食相关问题，忽略其他内容（不要emoji表情和毫不相干的废话）`;

      try {
        console.log("[ChatService] 调用 DeepSeek API...");
        ...
        console.log("[ChatService] DeepSeek API 调用成功");
      } catch (error) {
        console.error("[ChatService] DeepSeek API 调用失败，回退到本地实现", error);
        const response = this.generateAIResponse(dto.message);
        reply = response.reply;
      }
    } else {
      console.log("[ChatService] 未配置 DEEPSEEK_API_KEY，使用本地实现");
      const response = this.generateAIResponse(dto.message);
      reply = response.reply;
    }

    // 性能优化：异步保存 AI 回复（立即返回响应）
    this.saveAIMessage(userId, reply).catch((err) => {
      this.logger.error("保存 AI 回复失败", err);
    });

    console.log("[ChatService] 返回响应, reply 长度:", reply.length);
    return { reply };
  }
  ...
  private generateAIResponse(message: string): ChatResponse {
    ...
    // ========== 食谱/菜谱相关 ==========
    if (
      lowerMessage.includes("食谱") ||
      lowerMessage.includes("菜谱") ||
      lowerMessage.includes("做什么") ||
      lowerMessage.includes("吃什么") ||

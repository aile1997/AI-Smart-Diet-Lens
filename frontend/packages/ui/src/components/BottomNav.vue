...
<template>
  <view class="fixed bottom-0 left-0 w-full bg-white/95 backdrop-blur-xl border-t border-gray-200 pb-4 pt-3 px-6 z-50">
    <view class="flex justify-between items-center max-w-sm mx-auto">
      ...
      >
        ...
        <template v-if="item.isFab">
          <view
            class="w-14 h-14 rounded-full bg-[#38e07b] text-white shadow-lg shadow-[#38e07b]/40 flex items-center justify-center -mt-10 active:scale-95 transition-all border-4 border-white"
          >
            <text class="material-symbols-outlined text-3xl">{{ item.icon }}</text>
          </view>
        ...
        <!-- 普通标签 -->
        <template v-else>
          <text class="material-symbols-outlined text-3xl" :class="isActive(item.path) ? 'text-[#38e07b] fill-current' : 'text-gray-400'">
            {{ item.icon }}
          </text>
          <text class="text-[10px] font-medium" :class="isActive(item.path) ? 'text-[#38e07b]' : 'text-gray-400'">
            {{ item.label }}
          </text>
        </template>
      </view>

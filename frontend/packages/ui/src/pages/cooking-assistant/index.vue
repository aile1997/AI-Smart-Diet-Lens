...
const steps = ref<Step[]>([
  {
    ...
    image: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCEeOkuz8wFtmc8Le9AW9cFnaASz41uWIYEgBFEE6QAZDQywM2cKClq4WVWYLGlCsxo-r7qL3vokK5I5F0kIUU_BhTS-Q2E-JWPkZmdn5GUOaSwCsDRgfigz5BOPiCoJ4tsSLgl9Y7c5kz2O9IlEn61PXX0o0JBRr66U1C27aKg-1Drn5iayVMf71NYe7yNy_jZ3gZ2Jh3NEY129dQg-mYj7RAS4GhV6V0u8Ym3RnKv9EXAIhjtySJW-isymWQ4feDzXjsgvBzPVKF2'
  },
  {
    number: 2,
    title: '腌制调味',
  ...
  <view class="relative min-h-screen w-full flex flex-col bg-[#F5F7F8]">
    ...
    <view class="relative w-full h-[420px] shrink-0">
      ...
      <view class="absolute top-0 left-0 right-0 flex items-center justify-between p-6 pt-12 z-20">
        <view @tap="navigateBack" class="flex items-center justify-center w-10 h-10 rounded-full bg-black/20 backdrop-blur-md border border-white/10 active:bg-black/40 transition-colors">
          <text class="material-symbols-outlined text-white text-[24px]">arrow_back</text>
        </view>
        <view class="flex gap-3">
          <view class="flex items-center justify-center w-10 h-10 rounded-full bg-black/20 backdrop-blur-md border border-white/10 active:bg-black/40 transition-colors">
            <text class="material-symbols-outlined text-white text-[24px]">share</text>
          </view>
          <view class="flex items-center justify-center w-10 h-10 rounded-full bg-black/20 backdrop-blur-md border border-white/10 active:bg-black/40 transition-colors">
            <text class="material-symbols-outlined text-white text-[24px]">bookmark_border</text>
          </view>
        </view>
      </view>
      ...
      <view class="absolute bottom-0 left-0 w-full p-6 pb-2 z-10">
        <view class="flex items-center gap-2 mb-3">
          <text class="px-2.5 py-1 rounded-md bg-[#36e27b]/20 backdrop-blur-sm border border-[#36e27b]/30 text-[#36e27b] text-xs font-semibold tracking-wider uppercase">
            {{ recipe.tags[0] }}
          </text>
        ...
        <text class="text-3xl font-bold text-white leading-tight mb-2 tracking-tight">{{ recipe.name }}</text>
    ...
    <view class="relative flex-1 px-6 pb-24 -mt-2 z-10">
      ...
      <view class="flex gap-3 py-4 overflow-x-auto">
        <view class="flex h-9 shrink-0 items-center justify-center gap-2 rounded-lg bg-white border border-gray-100 px-4 shadow-sm">
          <text class="material-symbols-outlined text-[#36e27b] text-[18px]">schedule</text>
          <text class="text-[#111827] text-sm font-medium">{{ recipe.time }}</text>
        </view>
        <view class="flex h-9 shrink-0 items-center justify-center gap-2 rounded-lg bg-white border border-gray-100 px-4 shadow-sm">
          <text class="material-symbols-outlined text-[#36e27b] text-[18px]">signal_cellular_alt</text>
          <text class="text-[#111827] text-sm font-medium">{{ recipe.difficulty }}</text>
        </view>
        <view class="flex h-9 shrink-0 items-center justify-center gap-2 rounded-lg bg-white border border-gray-100 px-4 shadow-sm">
          <text class="material-symbols-outlined text-[#36e27b] text-[18px]">local_fire_department</text>
          ...
          <view class="flex flex-col items-center justify-center p-4 rounded-2xl bg-white border border-gray-100 shadow-sm relative overflow-hidden">
            ...
            <view class="relative w-12 h-12 mb-2">
              <svg class="w-full h-full -rotate-90" viewBox="0 0 36 36">
                ...
                <path class="text-[#36e27b] drop-shadow-[0_0_2px_rgba(54,226,123,0.3)]" :stroke-dasharray="`${recipe.proteinPercent}, 100`" fill="none" stroke="currentColor" stroke-width="3"></path>
              </svg>
      ...
      <view class="mt-8 mb-6 p-4 rounded-2xl bg-white border border-gray-100 shadow-lg flex items-center justify-between relative overflow-hidden">
        ...
        <label class="relative inline-flex items-center cursor-pointer z-10">
          <input
            ...
            @change="toggleVoiceAssistant"
            class="sr-only peer"
            type="checkbox"
          />
          <view class="w-11 h-6 bg-gray-200 rounded-full peer-checked:bg-[#36e27b] after:content-[''] after:absolute after:top-[2px] after:start-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full"></view>
        </label>
      </view>
      ...
      <view class="mt-2">
        <view class="flex items-center justify-between mb-4">
          <text class="text-lg font-bold text-[#111827]">所需食材 <text class="text-[#9CA3AF] font-normal text-sm ml-1">({{ recipe.ingredients.length }}项)</text></text>
          <view @tap="addToShoppingList" class="text-emerald-600 text-sm font-medium border border-emerald-600/30 px-3 py-1.5 rounded-lg flex items-center gap-1">
        ...
        <view class="space-y-3">
          ...
          >
            <view class="relative flex items-center justify-center shrink-0">
              <view class="w-5 h-5 rounded-full border border-gray-300 bg-transparent flex items-center justify-center">
                <text v-if="item.checked" class="material-symbols-outlined text-black text-xs">check</text>
              </view>
            </view>
      ...
      <view class="mt-8 pb-8">
        ...
        <view class="relative border-l-2 border-gray-200 ml-3 space-y-8">
          <view v-for="(step, index) in steps" :key="index" class="relative pl-8">
            <view class="absolute -left-[9px] top-0 w-4 h-4 rounded-full" :class="index === 0 ? 'bg-[#36e27b] shadow-[0_0_8px_rgba(54,226,123,0.5)]' : 'bg-white border-2 border-gray-300'"></view>
            ...
            <image
              v-if="step.image"
              :src="step.image"
              class="w-full h-32 rounded-lg bg-cover bg-center shadow-sm"
              mode="aspectFill"
            />
          </view>
        </view>
    ...
    <!-- Sticky Footer -->
    <view class="fixed bottom-0 left-0 right-0 p-4 pt-0 bg-gradient-to-t from-[#F5F7F8] via-[#F5F7F8] to-transparent z-30 max-w-[480px] mx-auto pointer-events-none">
      <view class="pointer-events-auto flex items-center justify-between gap-4 p-4 rounded-2xl bg-white/90 backdrop-blur-xl border border-gray-200/50 shadow-2xl">
        ...
        <view
          @tap="startCooking"
          class="flex-1 bg-[#36e27b] text-black font-bold text-base py-3 px-6 rounded-xl active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-[0_4px_12px_rgba(54,226,123,0.4)]"
        >

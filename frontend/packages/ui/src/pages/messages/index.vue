...
const filteredNotifications = computed(() => {
  ...
  return notifications.value.filter((n) => n.title.toLowerCase().includes(query) || n.content.toLowerCase().includes(query));
});
...
<template>
  <view class="relative w-full min-h-screen bg-[#F5F7FA]">
    <!-- 未登录状态 -->
    <view v-if="!isLoggedIn" class="flex flex-col items-center justify-center min-h-screen px-6 text-center">
      ...
      <view @tap="navigateToLogin" class="px-6 py-3 bg-primary text-white rounded-2xl font-bold"> 立即登录 </view>
    </view>

    <!-- 已登录状态 -->
    <template v-else>
      <!-- Header -->
      <view class="sticky top-0 z-30 px-4 pt-14 pb-4 bg-[#F5F7FA]/90 backdrop-blur-md flex items-center justify-between">
        <view
          @tap="navigateBack"
          class="w-10 h-10 -ml-2 rounded-full flex items-center justify-center text-slate-800 active:bg-black/5 active:bg-black/10 transition-colors"
        >

...
// 用户显示数据
const userName = computed(() => userStore.displayName || "访客");
...
const stats = computed(() => {
  ...
  return [
    { label: "连续记录", value: "0", unit: "天" },
    { label: "当前体重", value: "--", unit: "kg" },
    { label: "今日消耗", value: "0", unit: "kcal" },
  ];
});
...
onMounted(async () => {
  if (isLoggedIn.value) {
    await Promise.all([userStore.fetchProfile(), fetchDashboard()]);
  }
});
      ...
      <view class="sticky top-0 z-30 px-6 pt-14 pb-4 bg-[#F5F7FA]/90 backdrop-blur-md flex justify-between items-center">
        ...
        <view class="flex items-center gap-3">
          <view
            ...
            class="relative w-10 h-10 rounded-full bg-white shadow-sm flex items-center justify-center text-[#5a847b] active:scale-95 transition-transform"
          >
          ...
          <view class="flex items-center gap-5 relative z-10">
            ...
            <view class="flex-1">
              ...
              <view class="mt-2.5 flex items-center gap-1.5 bg-[#2f4540] px-3 py-1.5 rounded-full shadow-lg active:scale-95 transition-all w-fit">
                <text class="material-symbols-outlined text-base text-[#d4b368] filled">workspace_premium</text>
                <text class="text-[10px] font-bold tracking-wide text-white uppercase">PRO 会员</text>
                <text class="material-symbols-outlined text-sm text-white/60">chevron_right</text>
              </view>
            </view>
          </view>
        </view>
          ...
          <view class="bg-white rounded-2xl shadow-soft overflow-hidden divide-y divide-gray-50">
            ...
            >
              <view class="flex items-center gap-3.5">
                ...
                <text class="text-sm font-semibold text-[#1C1C1E]">{{ item.label }}</text>
              </view>
          ...
          <view class="bg-white rounded-2xl shadow-soft overflow-hidden divide-y divide-gray-50">
            ...
            >
              <view class="flex items-center gap-3.5 min-w-0 flex-1">
                ...
                <view class="min-w-0">
                  <text class="text-sm font-semibold text-[#1C1C1E] block">{{ item.label }}</text>
          ...
          <view class="bg-white rounded-2xl shadow-soft overflow-hidden">
            <view @tap="handleMenuClick(achievementMenu)" class="flex items-center justify-between p-4 active:bg-[#f4f7f5] transition-colors">
              <view class="flex items-center gap-3.5 min-w-0">
                ...
                <view class="min-w-0">
                  <text class="text-sm font-semibold text-[#1C1C1E] block">{{ achievementMenu.label }}</text>
          ...
          <view class="bg-white rounded-2xl shadow-soft overflow-hidden divide-y divide-gray-50">
            ...
            <!-- <view class="flex items-center justify-between p-4">
              <view class="flex items-center gap-3.5">
                ...
                <text class="text-sm font-semibold text-[#1C1C1E]">通知提醒</text>
              </view>
            ...
            <view class="flex items-center justify-between p-4 active:bg-[#f4f7f5] transition-colors">
              <view class="flex items-center gap-3.5">
                ...
                <text class="text-sm font-semibold text-[#1C1C1E]">语言</text>
              </view>
              <view class="flex items-center gap-2">
